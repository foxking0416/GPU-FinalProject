...
function makeConnectionLineGeometry( exporter, importer, value, type ){
	...
	//	start of the line
	var start = exporter.center; //A THREE.Vector3
	...
	var normal = (new THREE.Vector3()).sub(start,end); // This is like creating a vector from start to end
	normal.normalize();
...
function makeConnectionLineGeometry( exporter, importer, value, type ){
	...
	var distanceBetweenCountryCenter = exporter.center.clone().subSelf(importer.center).length();		
	//var distanceBetweenCountryCenter = exporter.center.distanceTo(importer.center);
